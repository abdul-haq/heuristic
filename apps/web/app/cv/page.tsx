...
export default function CvPage() {
    ...
    useEffect(() => {
        refresh();
        api<{ template: string | null }>('/bullets/template')
            .then(res => { if (res.template) setTemplate(res.template); })
            .catch(console.error);
    }, []);
    ...
    async function handleTemplateUpload(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        try {
            ...
            const res = await api<{ success: boolean; companiesFound: string[]; bulletCount: number }>(
                '/bullets/template',
                { method: 'POST', body: JSON.stringify({ template: text }) },
            );
            setTemplate(text);
            setTemplateResult(
                `Template saved — found ${res.companiesFound.length} companies (${res.companiesFound.join(', ')}) with ${res.bulletCount} bullets`,
            );
        } catch (e: any) {
            setErr(e.message);
        } finally {
        ...
        <main className="max-w-4xl mx-auto p-6 space-y-5">
            ...
            {/* <div className="bg-white border border-neutral-200 rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <label className={`px-4 py-2 text-sm font-medium rounded-lg cursor-pointer transition-colors ${uploading
            ...
            {/* <div className="bg-white border border-neutral-200 rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        <h2 className="text-sm font-medium">CV Template (.tex)</h2>
                        <p className="text-xs text-neutral-400 mt-1">
                            {template
                                ? 'Template uploaded. When you compile a tailored CV, your formatting and structure are preserved.'
                                : 'Upload your LaTeX CV template. The system will swap in tailored bullets while keeping your design.'}
                        </p>
                    </div>
                    <label className={`px-4 py-2 text-sm font-medium rounded-lg cursor-pointer transition-colors ${uploadingTemplate
                            ? 'bg-neutral-100 text-neutral-400'
                            : template
                                ? 'border border-neutral-300 text-neutral-600 hover:border-neutral-400'
                                : 'bg-neutral-900 text-white hover:bg-neutral-800'
                        }`}>
                        {uploadingTemplate ? 'Saving...' : template ? 'Replace template' : 'Upload .tex'}
                        <input
                            type="file"
                            accept=".tex"
                ...
                {template && !templateResult && (
                    <div className="text-xs text-emerald-600 font-medium">
                        ✓ Template ready — {template.length.toLocaleString()} characters
                    </div>
                )}
            </div> */}
            ...
            <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden">
                <div className="px-6 py-4 border-b border-neutral-100">
                    <h2 className="text-sm font-semibold">CV Setup</h2>
                    <p className="text-xs text-neutral-400 mt-0.5">One-time setup. Do both to enable the full workflow.</p>
                </div>
                ...
                <div className="grid grid-cols-2 divide-x divide-neutral-100">
                    ...
                    <div className="p-6">
                        <div className="flex items-start gap-3 mb-4">
                            ...
                            <div>
                                <p className="text-sm font-medium">Import bullets (PDF)</p>
                                <p className="text-xs text-neutral-400 mt-0.5">
                                    Upload your existing CV as a PDF. The AI extracts every bullet point, tags skills, and embeds them for RAG matching.
                                </p>
                            </div>
                        </div>

                        <label className={`inline-flex px-4 py-2 text-sm font-medium rounded-lg cursor-pointer transition-colors ${uploading
                                ? 'bg-neutral-100 text-neutral-400'
                                : 'bg-neutral-900 text-white hover:bg-neutral-800'
                            }`}>
                        ...
                        {uploading && (
                            <p className="text-xs text-neutral-400 animate-pulse mt-2">
                                Parsing bullets — takes 1-2 minutes...
                            </p>
                        )}

                        {uploadResult && (
                            <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 text-xs text-emerald-700 mt-3">
                                ✓ {uploadResult}
                            </div>
                        )}
                    </div>
                    ...
                    <div className="p-6">
                        <div className="flex items-start gap-3 mb-4">
                            ...
                            <div>
                                <p className="text-sm font-medium">Set template (.tex)</p>
                                <p className="text-xs text-neutral-400 mt-0.5">
                                    Upload your LaTeX CV. When you compile a tailored CV, your formatting, header, and design are preserved exactly.
                                </p>
                            </div>
                        </div>

                        <label className={`inline-flex px-4 py-2 text-sm font-medium rounded-lg cursor-pointer transition-colors ${uploadingTemplate
                                ? 'bg-neutral-100 text-neutral-400'
                                : template
                                    ? 'border border-neutral-300 text-neutral-600 hover:border-neutral-400'
                                    : 'bg-neutral-900 text-white hover:bg-neutral-800'
                        ...
                        {uploadingTemplate && (
                            <p className="text-xs text-neutral-400 animate-pulse mt-2">Saving template...</p>
                        )}
